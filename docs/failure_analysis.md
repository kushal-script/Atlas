...
The reference pipeline forms the search image by blurring the fine canvas and decimating it with area averaging, so each search pixel integrates a 10 by 10 block of specimen. The template was formed by bilinear point sampling at 10 times decimation, which reads one pixel in a hundred and retains aliasing the search image does not contain. Folding the decimation box filter into the template blur in quadrature with the beam spot was built as the fix, then measured in the August 12 template ablation and declined, so the shipped bank uses the bare spot sigma; the matched formation now lives in the raw confirmation stage, which applies the organisers' own box filter to the full reference before correlating. This is the same class of error as matching a sharp template against a soft image: the correlation peak survives but loses margin, and lost margin is exactly what turns an identifiable site into a near tie.
