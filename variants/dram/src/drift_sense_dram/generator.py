...
Every pair is DRAM, periodic word lines crossed at right angles by bit lines
with one storage node contact per crossing, so there is no style argument to
pass and no style to record beyond the one this variant can build."""

import json
import numpy as np
from PIL import Image, ImageDraw

from .geometry import build_dram_layout
...
def _scale_widths(dram_params, factor):
    """DRAM params with every drawn feature width scaled, pitches untouched.
    ...
    The contact radius is scaled with the line widths because the polygon scale
    degradation is a resist and etch bias that acts on every drawn edge.
    """
    from dataclasses import replace
    fields = {}
    for name in ("wl_duty", "bl_duty", "contact_radius_f"):
        lo, hi = getattr(dram_params, name)
        fields[name] = (min(lo * factor, 0.95), min(hi * factor, 0.95))
    return replace(dram_params, **fields)


def generate_pair(seed, cfg=None, modality="sem", absent=False, degrade=0):
    ...
    zones = build_dram_layout(mat, hgt, cfg.canvas.pixel_nm, rng_layout,
                              dram_params, target=(ru, rv), want=want)
    se_info = {}
    if modality == "sem":
    ...
    if degrade:
        ...
        sp2 = _scale_widths(dram_params, poly)
        search_src_mat = np.zeros((size, size), dtype=np.uint8)
        search_src_hgt = np.zeros((size, size), dtype=np.float32)
        build_dram_layout(search_src_mat, search_src_hgt, cfg.canvas.pixel_nm,
    ...
    if absent:
        ...
        zones = build_dram_layout(ref_mat, ref_hgt, cfg.canvas.pixel_nm,
                                  rng_layout2, dram_params,
                                  target=(ru, rv), want=want)
        if modality == "sem":
