...
Before the full resolution correlation both captures pass through a Laplacian
of Gaussian matched to the storage node contact, which is where the only
aperiodic content in a DRAM field lives; see MatchConfig.contact_emphasis.
...
from . import backend
from .params import REFERENCE_CAPTURE, DramParams


@dataclass
class MatchConfig:
    zoom: float = 10.0
    # Contact emphasis, the one place this variant departs from the shared
    # matcher. A DRAM field is a lattice of word lines crossed by bit lines and
    # is identical from one cell to the next, so the correlation surface is
    # degenerate at the pitch and the peak is free to lock onto the wrong cell.
    # The only content in the field that is not periodic is the small fraction
    # of contacts the process drops, together with the per contact size
    # variation the generator draws around each one, so those contacts are the
    # only evidence that can say which cell is the right one. A Laplacian of
    # Gaussian tuned to the contact scale weights the correlation towards the
    # dot lattice, where that evidence lives, and away from the line edges,
    # which are the same in every cell and can only support a degenerate match.
    # Turning it off restores the shared pipeline exactly.
    # Measured harmful and therefore off. Over 49 present DRAM pairs the filter lifts
    # nominal credit from 0.833 to 0.850 and costs degraded 0.648 to 0.560, a weighted
    # 0.731 to 0.691, because a Laplacian of Gaussian is a high pass and the degraded
    # captures are shot noise dominated at the frequency it emphasises. The code stays
    # so the result can be reproduced by setting this True.
    contact_emphasis: bool = False
    # Specimen contact radius in nanometres. Zero derives it from the
    # generator's own DRAM parameters, which is the derivation _contact_sigma_px
    # explains; set it directly for a process whose contacts are a different
    # size from the ones this repository generates.
    contact_radius_nm: float = 0.0
...
def _contact_sigma_px(cfg):
    """Laplacian of Gaussian scale, in search pixels, matched to one contact.

    The generator draws a storage node contact radius as contact_radius_f times
    the feature size, so the mid range radius is the product of the two range
    midpoints, about fifteen nanometres, and nothing here is a tuned constant.
    A Laplacian of Gaussian answers a disc of radius r most strongly when the
    zero crossing ring of its kernel falls on the disc edge, which for the two
    dimensional kernel is a sigma of r over root two. That scale is physical
    and the filter has to act in pixels, so it is divided by the size of a
    search pixel, the reference pixel size times the nominal magnification,
    giving about one pixel for a contact three search pixels across. One sigma
    serves both captures because the reference is resampled onto the search
    pixel grid before it is filtered, so by then both images have the same
    nanometres per pixel and the same physical scale is the same pixel count.
    """
    r_nm = float(cfg.contact_radius_nm)
    if r_nm <= 0.0:
        p = DramParams()
        r_nm = (sum(p.feature_nm) / 2.0) * (sum(p.contact_radius_f) / 2.0)
    search_px_nm = REFERENCE_CAPTURE.pixel_nm * cfg.zoom
    return float(r_nm / (np.sqrt(2.0) * search_px_nm))


def _contact_emphasis(x, sigma_px, cfg):
    """Laplacian of Gaussian, as a Gaussian followed by the discrete Laplacian.

    The kernel is deliberately left unnormalized. Normalized cross correlation
    is invariant to a positive scale factor on either input, so a scale
    normalization would change nothing and only the sign convention has to
    agree between the two captures, which it does because both are filtered
    here.
    """
    g = backend.gaussian(x, sigma_px, cfg.device)
    return cv2.Laplacian(np.ascontiguousarray(g, dtype=np.float32),
                         cv2.CV_32F, ksize=1)


def _make_template(ref_band, theta_deg, scale, cfg, emphasis_sigma_px=0.0):
    t = cfg.template_px
    ...
    # The emphasis filter needs real reference content past the template edge.
    # Without it the outermost template pixels would be filtered against a
    # mirrored copy of themselves while the search image's are filtered against
    # their true neighbours, and the two would stop being comparable exactly
    # where the template carries the least evidence. Three sigma of margin is
    # resampled with the template, filtered, then cropped away, so the returned
    # template is the same t pixels on the same grid whether or not the
    # emphasis is on: the sampled coordinate of an output pixel depends on its
    # distance from the centre, and cropping the margin symmetrically restores
    # every one of those distances.
    pad = int(np.ceil(3.0 * emphasis_sigma_px)) if emphasis_sigma_px > 0 else 0
    n = t + 2 * pad
    ...
    tmpl = affine_transform(ref_band, matrix, offset=offset,
                            output_shape=(n, n), order=1, mode="constant",
                            cval=float(ref_band.mean()))
    if pad:
        tmpl = np.ascontiguousarray(
            _contact_emphasis(tmpl, emphasis_sigma_px, cfg)[pad:pad + t, pad:pad + t])
    return tmpl
...
def locate(ref_img, search_img, cfg=None, return_artifacts=False, t_start=None):
    ...
    ref_bank = {s: backend.gaussian(ref, _effective_sigma(s, cfg), cfg.device) - ref_low
                for s in all_sigmas}
    search_band, search_noise = _preprocess(search_img, cfg, denoise=True)
    # The emphasis is applied at full resolution and nowhere else. The
    # prescreen decimates by four, which puts a contact, three search pixels
    # across at the nominal magnification, under its Nyquist limit, so a filter
    # tuned to the contact would have nothing left to answer to there and would
    # emphasise its aliases instead. The coarse stage therefore screens the
    # unfiltered band, as it always has, and the hypotheses it hands on are
    # rescored at full resolution where the dot lattice is resolved.
    emphasis_sigma = _contact_sigma_px(cfg) if cfg.contact_emphasis else 0.0
    search = (_contact_emphasis(search_band, emphasis_sigma, cfg)
              if emphasis_sigma > 0 else search_band)
    ...
    def evaluate(sig, theta, scale):
        ...
        if key in tried:
            return tried[key]
        tmpl = _make_template(ref_bank[sig], theta, scale, cfg, emphasis_sigma)
    ...
    small = cv2.resize(search_band, (search_band.shape[1] // ds,
                                     search_band.shape[0] // ds),
    ...
    for _dth in (-cfg.refine_rot_step_deg, cfg.refine_rot_step_deg):
        _tp = _make_template(ref_bank[sigma_best], theta_best + _dth, scale_best,
                             cfg, emphasis_sigma)
        _rp = corr.full(_tp)
    ...
    diag = {
        ...
        "contact_emphasis_sigma_px": float(emphasis_sigma),
...
def optical_config():
    # No contact emphasis on the optical path. A fifteen nanometre contact is
    # an order of magnitude under the diffraction limit of a visible light
    # capture, so the dot lattice is not in those pixels to be emphasised and
    # a filter at that scale would only amplify the noise sitting where the
    # signal is not.
    return MatchConfig(psf_sigma_bank_nm=(2.0, 8.0),
                      bandpass_sigma_px=120.0,
                      denoise_sigma_px=1.5,
                      contact_emphasis=False)
