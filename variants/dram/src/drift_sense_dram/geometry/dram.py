    ...
    for (v0, v1) in v_edges:
        for (u0, u1) in u_edges:
            ...
            if len(bls) == 0 or len(wls) == 0:
                continue
            # The contact sits at the centre of the cell the two lines bound,
            # half a pitch from each, rather than on the crossing point itself.
            # That is where a 6F2 storage node contact goes, and it is the only
            # placement that leaves the dot visible: painted onto a bit line it
            # would be tungsten on tungsten and would carry no material
            # contrast, which would cost the published specification's high
            # contrast dot the contrast it asks for. The lattice the
            # specification describes is unchanged, one dot per word line and
            # bit line crossing at the crossing pitch; only its phase within
            # the cell differs.
