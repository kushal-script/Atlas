"""Drift Sense dataset generation entry point.
...
def main():
    ...
    ap.add_argument("--generator",
                    ...
                    default="physics")
    # This variant builds one architecture, so the only style it accepts is the
    # one it can produce. The flag stays rather than being deleted because a
    # harness that already passes --style finfet must keep working, while a
    # request for dram or mixed is now refused instead of quietly ignored.
    ap.add_argument("--style", choices=["finfet"], default="finfet",
                    help="physics generator only, and finfet is its only "
                         "architecture: the other generators carry their own "
                         "fixed structure and record what they actually produced")
