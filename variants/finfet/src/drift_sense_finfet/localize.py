...
tone conventions still match.

The matched surface is the sum of two correlation channels. This variant's
architecture is dense parallel vertical fins crossed by sparse horizontal gate
bars, so the image is nearly invariant under vertical translation and the full
image correlation forms a ridge: it fixes x and the pose and leaves y resting
on whatever gate structure survived the capture. The second channel removes the
fin component, by subtracting each column's own mean over the window, and
correlates what is left, which is the gate bars, the diffusion breaks and the
trench contacts. That surface is the transposed ridge, and the two sum to a
peak at their crossing.

Candidate peaks within a tolerance of the best score are collected; a
residual disambiguation stage
...
class MatchConfig:
    ...
    # Gate emphasis, the second correlation channel. With the fins running
    # vertically the search image is nearly invariant under vertical
    # translation, so the full image correlation forms a ridge: fin phase fixes
    # x and the pose, and only the sparse horizontal gate structure fixes y,
    # which is the first thing degradation erases. The second channel removes
    # the fin component and correlates what is left, and the two surfaces are
    # summed. Kept behind a flag, default on for this variant, so the shipped
    # single channel path is still reachable and the difference is measurable.
    # Off, because the premise it was built on is false. The channel was designed to
    # resolve a correlation ridge along the fin axis, but the peak measures 1 by 2
    # pixels at the true pose, isotropic rather than ridged, and on severe pairs the two
    # channels peak at sites 480 px apart so the blend averages disagreement. The code
    # stays so the result can be reproduced by setting this True.
    gate_emphasis: bool = False
    # Weight of the gate channel in the combined surface. The two channels are
    # complementary ridges: channel one is dominated by the fin lattice, which
    # is invariant along the fins, so it fixes x and says almost nothing about
    # y; channel two has had exactly that invariant component removed, so what
    # is left varies along y and hardly at all along x, and it fixes y and says
    # almost nothing about x. Both are normalized correlations, so they already
    # share a scale and neither needs rescaling onto the other. Two ridges at
    # right angles sum to a single peak at their crossing only while they are
    # added with comparable weight; move the weight either way and the maximum
    # of the sum slides back along the stronger ridge, which is the failure the
    # channel exists to remove. The symmetric weight is what the geometry asks
    # for rather than a constant a sweep happened to land on, and it stays a
    # config field so a sweep can still contradict it.
    gate_emphasis_weight: float = 0.5
    # A reference crop can land where no gate bar, diffusion break or contact
    # falls, and the gate channel is then correlating the residue of the fin
    # removal against noise. The channel is skipped when the template's column
    # demeaned energy falls below this fraction of its total zero mean energy,
    # so a crop with no y structure keeps the single channel answer instead of
    # having noise mixed into it.
    gate_emphasis_min_energy: float = 0.02
    reranker_path: str = ""
...
def _peak_prominence(surface, peak=None):
    """How far a surface's peak stands clear of the surface it sits on.

    The raw correlation value falls with noise, so it cannot say whether a peak
    is meaningful; this asks whether the peak is unlike its own surface, which
    is scale free and therefore comparable between two correlation channels
    whose absolute levels differ. Returns the prominence with the median and
    the median absolute deviation it was measured against.
    """
    flat = np.asarray(surface).ravel()
    med = float(np.median(flat))
    mad = float(np.median(np.abs(flat - med)))
    top = float(flat.max()) if peak is None else float(peak)
    return (top - med) / max(1.4826 * mad, 1e-6), med, mad


def _gate_emphasis_map(search, tmpl, cfg):
    """Normalized correlation with the fin component removed from both sides.

    With the fins running vertically, the fin content is exactly the part of
    the image that does not vary down a column, so subtracting each column's
    own mean removes it and leaves the horizontal gate bars, the diffusion
    break bands and the trench contacts, which are the only structure that
    fixes y.

    The subtraction is done per window rather than once over the whole frame.
    A profile averaged down all thousand rows would remove nothing at the
    disclosed poses: five degrees of rotation carries a fin eighty seven pixels
    sideways over that height, some twenty eight fin pitches at a three pixel
    pitch, so the profile would average the lattice away. Over the ninety row
    window the same tilt carries it about eight pixels, two to three pitches,
    so the removal is partial at the extreme of the disclosed range and exact
    at the nominal pose where the fins are upright. What survives survives identically on both sides, because
    template and window are demeaned over the same window, so a partial removal
    costs the channel some of its purity and none of its position.

    The numerator needs no correction term. The column demeaned template sums
    to zero down every column, so correlating it against the raw search image
    already gives the covariance of the two demeaned fields, and the large fin
    term is never formed and then subtracted. The denominator is each window's
    own residual energy, taken from box sums in double precision because there
    the cancellation is unavoidable and the residual is a small part of the
    whole.

    Returns a surface shaped like the full resolution correlation, or None when
    the reference crop carries too little structure along y to be worth asking.
    """
    th, tw = int(tmpl.shape[0]), int(tmpl.shape[1])
    h, w = int(search.shape[0]), int(search.shape[1])
    if th < 2 or tw < 1 or h < th or w < tw:
        return None
    t64 = np.asarray(tmpl, dtype=np.float64)
    t0 = t64 - t64.mean(axis=0, keepdims=True)
    e_t = float(np.sum(t0 * t0))
    e_full = float(np.sum((t64 - t64.mean()) ** 2))
    if e_full <= 0.0 or e_t < cfg.gate_emphasis_min_energy * e_full:
        return None

    num = cv2.matchTemplate(np.ascontiguousarray(search, dtype=np.float32),
                            np.ascontiguousarray(t0, dtype=np.float32),
                            cv2.TM_CCORR).astype(np.float64)

    s64 = np.asarray(search, dtype=np.float64)
    i2 = cv2.integral(s64 * s64)
    box_s2 = i2[th:, tw:] - i2[:-th, tw:] - i2[th:, :-tw] + i2[:-th, :-tw]
    col = np.cumsum(s64, axis=0)
    col = np.vstack([np.zeros((1, w)), col])
    v = col[th:, :] - col[:-th, :]
    row = np.cumsum(v * v, axis=1)
    row = np.hstack([np.zeros((v.shape[0], 1)), row])
    e_w = box_s2 - (row[:, tw:] - row[:, :-tw]) / float(th)
    np.clip(e_w, 1e-9, None, out=e_w)
    out = num / np.sqrt(e_w * e_t)
...
def locate(ref_img, search_img, cfg=None, return_artifacts=False, t_start=None):
    ...
    # the answer. It stays on the full image channel it was measured on, so
    # that turning the second channel on does not silently redefine a feature
    # the presence model was fitted against.
    _py0, _px0 = np.unravel_index(int(np.argmax(resp_fin)), resp_fin.shape)
    _stab = []
    ...
    # Channel two. The pose is already settled, and it was settled on the dense
    # fin evidence, which is what determines it; the gate channel is asked only
    # where the match sits, at that one pose, so it costs one correlation and
    # not another pass over the hypothesis grid.
    resp = resp_fin
    gate_weight = 0.0
    gate_score = 0.0
    resp_gate = None
    prom_fin, _, _ = _peak_prominence(resp_fin)
    prom_gate = -1.0
    if cfg.gate_emphasis and cfg.gate_emphasis_weight > 0.0:
        resp_gate = _gate_emphasis_map(search, tmpl_best, cfg)
        if resp_gate is not None and resp_gate.shape == resp_fin.shape:
            gate_score = float(resp_gate.max())
            gate_weight = float(np.clip(cfg.gate_emphasis_weight, 0.0, 1.0))
            resp = (1.0 - gate_weight) * resp_fin + gate_weight * resp_gate
            # Recorded per channel so the two can be compared pair by pair
            # without rerunning. Damping the gate channel by the ratio of these
            # two was written and then removed: the ratio came out above one on
            # every pair measured, 1.14 and 1.53 nominal and 1.31 and 1.32 at
            # severity four, so the damping clipped to no effect and would have
            # shipped as a safeguard that never fires. The reason it comes out
            # that way is itself the evidence for this channel. The fin channel
            # is a ridge, and a ridge is a high plateau that lifts its own
            # surface's median, so its peak stands less clear of it; removing
            # the fin component removes the plateau and leaves a peakier
            # surface. The channel that looks weaker by raw correlation is the
            # more decisive one.
            prom_gate, _, _ = _peak_prominence(resp_gate)
        else:
            resp_gate = None
    ...
    # actually asking.
    peak_prominence, _med, _mad = _peak_prominence(resp, score_best)
    _p99 = float(np.quantile(resp, 0.99))
    ...
    diag = {
        ...
        "gate_emphasis_weight": float(gate_weight),
        "gate_channel_score": float(gate_score),
        "fin_channel_score": float(resp_fin.max()),
        "fin_channel_prominence": float(prom_fin),
        "gate_channel_prominence": float(prom_gate),
    ...
    if return_artifacts:
        diag["artifacts"] = {"search": search, "template": tmpl_best,
                             "resp": resp, "resp_fin": resp_fin,
                             "resp_gate": resp_gate,
                             "r2": r2, "med": med, "rt0": rt0,
                             "wide": wide}
