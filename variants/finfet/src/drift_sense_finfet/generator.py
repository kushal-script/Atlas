...
from .geometry import build_finfet_layout
...
def _scale_widths(style_params, factor):
    ...
    fields = {}
    for name in ("fin_width_frac", "gate_width_frac"):
        lo, hi = getattr(style_params, name)
        fields[name] = (min(lo * factor, 0.95), min(hi * factor, 0.95))
    return replace(style_params, **fields)


def generate_pair(seed, style="finfet", cfg=None, modality="sem", absent=False, degrade=0):
    """One reference and search pair of the variant's single architecture.

    The style argument survives so a caller written against the two
    architecture pipeline still binds, but this variant builds FinFET only and
    says so rather than quietly returning something else.
    """
    if style != "finfet":
        raise ValueError(f"this variant builds finfet layouts only, not {style!r}")
    ...
    zones = build_finfet_layout(mat, hgt, cfg.canvas.pixel_nm, rng_layout,
    ...
    if degrade:
        ...
        build_finfet_layout(search_src_mat, search_src_hgt, cfg.canvas.pixel_nm,
    ...
    if absent:
        ...
        zones = build_finfet_layout(ref_mat, ref_hgt, cfg.canvas.pixel_nm,
