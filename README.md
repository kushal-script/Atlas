...
`docs/phase2_failure_analysis.md` carries the full analysis, including the shortcut our own generator nearly taught the model: the first fit put its largest weight on measured image noise, because every absent pair in the first suite happened to be a clean capture. The suite generator now degrades half of its absent pairs across the same severity ladder, which removed the shortcut and forced the decision onto structural evidence.
