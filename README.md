...
| Runtime | 3.7 s per pair on one CPU core, measured through the entry point on a representative mix |
...
| --- | --- | --- | --- |
| Localization | 40 | 23.3 | credit 0.74 nominal, 0.46 degraded |
| Pose recovery | 20 | 18.3 | scale credit 0.88, rotation credit 0.95 |
| Rejection | 15 | 7.7 | reject class F1 0.52, precision 0.42, recall 0.67 |
| Confidence calibration | 10 | 9.0 | AUC 0.90 |
| Core total | 85 | 58.4 | |

Rotation lands within a quarter degree, the full credit band, on 86 percent of held out pairs after a final euclidean alignment polish; the motion model is euclidean rather than affine on purpose, because an affine fit on a periodic lattice slides scale by pitch fractions and measurably degraded the scale estimate. Runtime through the entry point is 3.7 s per pair on one CPU core over a representative mix, and a wall clock guard drops the optional wide grid and residual stages as a pair's budget runs down, because a pair that overruns the scored timeout is worth nothing at all.
...
The optical bonus set is scored separately, since it gates on a localization credit of 0.40 with the grayscale sets at 0.50. Two independently generated RGB brightfield suites under disjoint master seeds return credit 0.43 and 0.63, both clear of the gate, against a weighted grayscale credit of 0.58. The spread between the two optical suites is the sampling variance of small sets rather than a difference in method, and the lower of the two is the number to plan against.
...
.venv/bin/python scripts/package_submission.py --python311 /path/to/python3.11 \
    --pairs data/p2holdout --extra submission/failure_analysis.pdf
```
