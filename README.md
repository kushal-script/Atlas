...
| Runtime | median 3.0 s per pair and 6.1 s worst case on one CPU core, measured through the entry point on a representative mix |
...
The decision is therefore a small logistic model over the evidence the localizer already produces, shipped with the submission as `models/presence_model.json` and loaded by the entry point at run time. It reads fifteen quantities, of which fourteen carry weight in the shipped fit and one, whether the wide pose search beat the nominal pose, was constant through that fit because of a string comparison bug and therefore carries a weight of exactly zero; the comparison is now correct and the feature stays inert until a refit gives it a weight, so no shipped decision depends on it. Among the fourteen are the peak and its prominence above the correlation surface, how degenerate that surface was through the strict and wide candidate counts, the measured noise, the deviation field's margin in robust units after the shared periodic content is projected out, and quadrant consistency: each quadrant of the matched template re correlated independently in a small window around the winning site. That last signal is the one an impostor cannot fake. A true match pins all four quadrants to the same offset because their aperiodic content agrees quadrant by quadrant, while an impostor matches only the shared lattice, so any lattice aligned offset serves any quadrant equally and the four best offsets scatter across cells. Against the best single feature at reject class F1 0.46, the combination reaches 0.655 on a held out suite.
...
**Runtime, and the defect measuring it properly exposed.** Through the entry point the median is 3.0 s per pair and the maximum 6.1 s over the proportional holdout with the absent pairs included, which are the slowest in the mix because a weak correlation peak is what triggers the width rescue. Capping OpenCV to four threads to match the reference machine's core count moves the median by 0.15 s and the credit not at all, so the pipeline is effectively single threaded and the four core limit costs nothing; what the reference machine will cost is per core speed.
