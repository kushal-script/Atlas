...
**How the score column is computed.** The presence decision thresholds the best normalized correlation peak. The reported score is the confidence in the decision actually made, not in presence itself: the peak's distance from the threshold is mapped through a logistic to a presence probability, the confidence is the larger of that probability and its complement, so a pair rejected on a very weak peak is a confident rejection, and it is damped when the correlation surface was degenerate, using the same wide candidate count and peak prominence statistics that drive the Phase 1 confidence regimes, and a found row is further damped by how many of the four template quadrants agreed on the site, which tracks localization correctness. The scale is monotonic in the probability that the row is right, which is what a ranking metric needs; no fixed range is claimed.
