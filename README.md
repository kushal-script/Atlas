...
The Phase 2 submission is complete, packed and self tested: `submission_phase2.zip` carries the entry point, its sources, every model weight, the frozen 3.11 requirements, the documented generator and the two page failure analysis, and the packager proves the archive by extracting it, running it with sockets blocked and asserting the full output contract. Measured through the entry point itself, the current build scores an estimated 79 to 82 of the 85 core points on suites generated from the organisers' released pipeline (five post freeze surprise seeds average 78.8, range 76.6 to 81.7, with the spread living almost entirely in the rejection block's eight absent pairs per suite), 59 to 68 on this repository's own deliberately harsher generator, and on the organisers' shared 20 pair sample, kept as a pure test throughout, present pair credit 0.988 against their own baseline's 0.800 with fifteen of sixteen pairs at full credit. Robustness is probed past the disclosed problem: a severity rung beyond the released ladder still earns 0.68 credit while the score column keeps ranking wrong answers below right ones, six deliberate appearance family breaks localize at 1.000 on five and the sixth exposed a polarity defect that is fixed, floored and covered by the suite that found it, and four rounds of external audit have been verified finding by finding, with every genuinely new signal closed by a measured change: an auto calibrated correlator choice, a slow machine budget, an incrementally flushed output file, and the platform test fixes. The one recorded residual weakness is the identical zone geometry decoy class, grabbed at about six in ten, priced into the threshold selection rather than hidden.
...
**The current build, across every family it can be measured on.** On suites generated fresh from the organisers' released pipeline, five 40 pair surprise suites whose seeds were chosen only after every fitted decision was frozen average an estimated 78.8 of the 85 core points, spread 76.6 to 81.7, with localization tight at 36.2 to 39.7 of 40 and pose at 19.0 to 19.6 of 20 across every seed while nearly all the spread lives in the rejection F1 of eight absent pairs per draw. The two pre freeze battery suites on the same recipes read 79.2 and 81.5, inside that band. On this repository's own generator, whose physics are deliberately harsher than the released ones, the two holdout suites read 68.4 and 59.2, the documented cross distribution floor. On the organisers' shared 20 pair sample, never fitted on, present credit is 0.988 against 0.800 for their own brute force baseline, with both severity 3 pairs their baseline zeroes at full credit and the single severity 4 pair recovered from a 426 px miss to 0.31 px by the raw confirmation override. The alien distribution suite, the released pipeline's geometry under six deliberate appearance breaks, localizes at exactly 1.000 on five families; the sixth found the one confidently wrong behaviour in the system, inverted contrast reaching the raw statistic unhandled, which is fixed, floored at a raw peak of 0.25, and reads 1.000 since. A severity five rung the released verifiability gate can barely generate earns 0.68 credit with no false rejects and wrong grabs scored at a median 0.514 against 1.000 for correct ones, so past the disclosed ladder the pipeline loses credit rather than lying (`experiments/20260901_stress_and_decoys`, `experiments/20260901_alien_distribution`, `experiments/20260901_organiser_sample_validation`).
